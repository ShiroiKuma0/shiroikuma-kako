...
**📥 Latest release: [`155.0.1+036`](https://github.com/ShiroiKuma0/shiroikuma-kako/releases/latest)** — [all releases & downloads »](https://github.com/ShiroiKuma0/shiroikuma-kako/releases)
...
Pick a backup directory once; the page then shows the newest export in it every time you open it. One panel exports — or restores — the browser by category, **nineteen of them**: the 白い熊 UI theme, imported fonts, Firefox's own settings, bookmarks, saved passwords, credit cards and addresses, open tabs, browsing history, the search engine you chose and your Firefox Account sign-in — and, since `155.0.1+036`, **`about:config` preferences, custom search engines, pinned shortcuts, site permissions, the never-save-a-password list, tab collections, and the extensions themselves with everything stored inside them**.

The point of that list is that almost none of it lives where you would guess. Fenix keeps its settings in **two** preference files, not one — 82 keys in the file a backup used to read, 112 in the one it did not. A custom search engine is an XML file, not a preference. `about:config` is Gecko's own `prefs.js`. Pinned shortcuts, site permissions, the never-save list and collections are four separate databases. The settings category now sweeps every preferences file the app has, so the one upstream adds next cycle travels without anyone noticing it exists — while the key to your password database, this app's automation token and one-shot migration stamps are held back, because restoring those breaks the phone they land on.

**Your extensions come back as extensions, not as a shopping list.** The archive used to record which add-ons were installed and leave the restore to re-download each from AMO — needing the network, needing AMO to answer, needing every add-on to still be listed. The XPIs travel now, and the restore installs from the file: the exact builds that were backed up, offline. Their storage travels with them, split so a routine backup can skip the bulk — **extension settings** (each add-on's own options) and **extension databases** (everything in `indexedDB`, which for a dictionary add-on is gigabytes). Restored add-ons approve themselves silently, since you already granted those permissions on the phone the backup came from.

Making that work meant getting a detail exactly right: Gecko names each extension's storage after a `moz-extension` UUID it mints per profile, so restored data belongs to nobody unless the add-on comes up under the UUID it was backed up with — and that cannot be arranged afterwards without leaving the add-on installed, enabled and unable to serve a single one of its own files. So a restore stages everything and installs nothing; the UUIDs go into the profile with the engine down, and the add-ons are installed after, adopting them.

The archive is a ZIP of readable JSON — no databases, no opaque blobs — plus the extension files themselves, and it is read back as a stream, so a multi-gigabyte restore never has to fit in memory. Two limits are honest and inherent: **visit times do not survive** a round trip, because the history store offers no timestamped write, so a restored history is dated to the restore; and **per-tab engine state** — scroll offsets, form contents — is the engine's opaque blob, so tabs come back unloaded at the top of the page.

Credentials necessarily travel as plaintext inside the archive, which the panel says out loud — and that includes the account's refresh token, so anything that can read a backup can sign in as you. Treat the file like the passwords it holds, and encrypt it wherever it is stored.
